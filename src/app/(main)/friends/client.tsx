  ...
  return (
    <div className="container mx-auto space-y-8 px-4 py-8">
      ...
      <section className="rounded-lg border bg-card p-6 shadow-sm">
        ...
        <form className="mt-4 grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
          ...
          <div className="md:col-span-2 flex items-center justify-end gap-3">
            ...
            <Button type="submit" disabled={requestFriendMutation.isPending}>
              Envoyer une demande
            </Button>
          </div>
        </form>
      ...
      <FriendsLists
        ...
        isError={isError}
        error={error}
        respondMutationPending={respondMutation.isPending}
        cancelMutationPending={cancelMutation.isPending}
        removeFriendPending={removeFriendMutation.isPending}
        onRemoveFriend={(friendId) => removeFriendMutation.mutate(friendId)}
        onCancelRequest={(requestId) => cancelMutation.mutate(requestId)}
        onRespondRequest={(requestId, accept) =>
          respondMutation.mutate({ id: requestId, accept })
        }
      />
    </div>
  );
...
interface FriendsListsProps {
  ...
  incoming: GraphQLFriendRequest[];
  outgoing: GraphQLFriendRequest[];
  isLoading: boolean;
  isError: boolean;
  error: unknown;
  respondMutationPending: boolean;
                  ...
                  <tr key={entry.id}>
                    ...
                    <td className="px-4 py-3 text-sm">
                      <Button
                        ...
                        onClick={() => onRemoveFriend(entry.friend.id)}
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      disabled={respondMutationPending}
                    >
                      Refuser
                    </Button>
                    <Button
