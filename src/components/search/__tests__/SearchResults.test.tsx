...
describe("SearchResults", () => {
  ...
  it("shows loading skeleton when fetching", () => {
    ...
    const { container } = render(<SearchResults query="demo" />);
  ...
  it("renders error message on failure", () => {
    ...
    render(<SearchResults query="demo" />);
    expect(
      screen.getByText("Une erreur est survenue lors de la recherche.")
    ).toBeInTheDocument();
  });
  ...
  it("displays results and active filters", () => {
    ...
    expect(screen.getByText('Résultats pour "demo"')).toBeInTheDocument();
